...
// General Settings - in definition.js
...
function mow_addon_contacts_add_results_to_email_text(showParty, showPersonal) {
  ...
  if (showPersonal > 0) {
    ...
    for (i = 0; i <= intQuestions - 1; i++) {
      ...
      if (questionPoints >= 99)
        statistics_text += `\n ${i + 1}. ${questionShort}: Übersprungen`;
      else if (questionPoints >= 1)
        statistics_text += `\n ${i + 1}. ${questionShort}: Zustimmung`;
      else if (questionPoints == 0)
        statistics_text += `\n ${i + 1}. ${questionShort}: Neutral`;
      else statistics_text += `\n ${i + 1}. ${questionShort}: Ablehnung`;
    } // end: for-i
  } // end: if showPersonal
...
function mow_addon_contacts_write_buttons_to_dom(partyNum, datasetNameValue) {
  ...
  divContent +=
    '<div class="row" id="resultsShortPartyAddonContactsInResults' +
    partyNum +
    '">';
  ...
  if (CR_CONTACT_ACTIVE_EMAIL > 0) {
    ...
    if (datasetNameValue) {
      strMailAddress =
        mow_addon_contacts_validate_email_address(datasetNameValue);
      strMailSubject =
        encodeURIComponent(CR_CONTACT_SUBJECT_EMAIL_DATATAG) +
        "" +
        encodeURIComponent(arPartyNamesLong[partyNum]);
      strMailBody =
        encodeURIComponent(CONTACT_TEXT_EMAIL_DATATAG) +
        "%20" +
        mow_addon_contacts_add_results_to_email_text(0, 1); // zeige nur pers. Ergebnis (0,1)
    } else {
      strMailAddress = mow_addon_contacts_validate_email_address(
        CR_CONTACT_ADDRESS_EMAIL
      );
      strMailSubject =
        encodeURIComponent(CR_CONTACT_SUBJECT_EMAIL_DEFAULT) +
        "" +
        encodeURIComponent(arPartyNamesLong[partyNum]);
      strMailBody =
        encodeURIComponent(CONTACT_TEXT_EMAIL_DEFAULT) +
        "%20" +
        mow_addon_contacts_add_results_to_email_text(1, 1); // zeige Partei und pers. Ergebnis (1,1)
    }
    ...
    divContent +=
      ...
      "&amp;body=" +
      strMailBody +
      '" role="button" class="btn btn btn-success" target="_blank" rel=noopener noreferrer>' +
  ...
  document
    .querySelector(
      `#resultsShortPartyClamp${partyNum} #containerForAddonGeneratedElements`
    )
...
// Starte die Suche nach dem DATASET mit Wert XYZ im <SPAN>
// und schreibe dann die Kontakt-Buttons
function mow_addon_contacts_search_dataTag_in_partyDescription(datasetName) {
